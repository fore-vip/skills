...
- 模板位置：`templates/driver-mcu.md`（SKILL 自带，只读）
- 启用方式：在 `~/.iot/config/config.json` 的 `drivers[]` 里声明 `type: serial` 或 `type: ble`；需要改造时复制到 `~/.iot/drivers/mcu.md` 再改
...
按帧格式解析 `PROP,VAL,TS`，转 `{val, unit, ts}` 并回写 `~/.iot/config/registry.json`。读超时（默认 2s）报无响应。
