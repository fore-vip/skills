...
- 模板位置：`templates/driver-system.md`（SKILL 自带，只读）
- 启用方式：在 `~/.iot/config/config.json` 的 `drivers[]` 里声明 `type: system`
- 适用：控制用户**电脑本身**的外设与系统状态——音量、屏幕亮度/开关、电源（休眠/关机）、麦克风/摄像头占用、风扇转速等
- 通信方式：宿主系统 API 或 CLI（macOS/Windows/Linux 差异由中控做平台判断）

## 连接信息

无需外部连接，`type: system`，`config: { platform: "auto" }`。中控运行时按宿主平台选实现。
