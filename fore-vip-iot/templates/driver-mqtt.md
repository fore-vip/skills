...
- 模板位置：`templates/driver-mqtt.md`（SKILL 自带，只读）
- 启用方式：在 `~/.iot/config/config.json` 的 `drivers[]` 里声明一条 `type: mqtt`；需要改造时复制到 `~/.iot/drivers/mqtt.md` 再改
...
设备收到 `set` 后回 `state` 主题，中控解析为 `{val, unit, ts}` 并回写 `~/.iot/config/registry.json`。
...
- 凭证走环境变量，不写进 `~/.iot/config/config.json` 明文，更不进 SKILL 分发包。
